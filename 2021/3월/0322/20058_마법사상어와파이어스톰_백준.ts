...
function solution(): void {
  ...
  let iceArray: number[][] = Array.from(Array(2 ** N), () =>
    Array(2 ** N).fill(0)
  );
        ...
        while (gStartCol < 2 ** N) {
          for (let gRow = gStartRow; gRow < gStartRow + 2 ** L; gRow++) {
            for (let gCol = gStartCol; gCol < gStartCol + 2 ** L; gCol++) {
              iceArray[gCol - gStartCol + gStartRow][
                2 ** L - 1 - (gRow - gStartRow) + gStartCol
              ] = copyIceArray[gRow][gCol];
            }
          }
          gStartCol += 2 ** L;
        }
  ...
  function findMaxIceLand(N: number, array: number[][]): number {
    ...
    let visited: boolean[][] = Array.from(Array(2 ** N), () =>
